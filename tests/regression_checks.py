...
require("server once-only projects keep all-day completion semantics", "if (['npc', 'number'].includes(id)) return Math.max(0, target - (state.counts[id] || 0));" in text and "const repeatableProjectStateKeys" in text)
...
daily_ui = text.index("每日项目（分时执行）", top_start)
...
require("daily split schedule note is shown in the section title", '<summary>每日项目（分时执行）<span class=\"summary-note\">06:00重置；酒吧10:05；好友午饭后；搬家不执行</span></summary>' in text)
...
require("daily friend and bar allow high-count cross-page runs", "maxPhaseAgeMs: 20 * 60 * 1000" in text and "maxPhaseAgeMs: 15 * 60 * 1000" in text)
require("scheduled friend rounds reset traversal but preserve daily progress", "friendState.visited = [];" in text and "friendState.page = 1;" in text and "friendState.counts = {}" not in text)
...
require("facility remembers completed setup before re-entering a shop", "state.renewed.includes(target.propId)" in facility_run and "本轮已使用，离开残留设置页" in facility_run and "本轮不重复使用" in facility_run)
return_store_start = facility_run.index("async returnFromStore(state, target)")
return_store_end = facility_run.index("async run()", return_store_start)
facility_return_store = facility_run[return_store_start:return_store_end]
require("facility store returns to its saved allowlisted target", "allowedReturnPaths" in facility_return_store and "location.assign(returnPath)" in facility_return_store and "\n      history.back();" not in facility_return_store)
require("facility normalizes persisted round arrays", "state.renewed ||= [];" in facility_run and "state.checked ||= [];" in facility_run and "state.purchases ||= {};" in facility_run)
...
require("cup reward is claimed before final completion", cup_reward < bar_complete and "已点击猜酒杯真实领奖/停止按钮" in daily_bar_run)
...
require("late friend recovery does not repeat an already finished compound", "const compoundStillPending = phase.id === 'energy' || energyLast > compoundLast;" in text and "&& compoundStillPending && isEnabled('foodCompound')" in text)
...
require("friend is removed from the old breakfast fixed schedule", "id: 'dailyFriend', module: 'dailyFriend'" not in daily)
require("bar uses daily 10:05 plus up to three minutes", "id: 'dailyBar', module: 'dailyBar'" in daily and "slot: '10:05', jitterMin: 0, jitterMax: 3" in daily)
...
require("v370 timing and facility migration exists", "v370_daily_timing_facility_migrated" in text and "sched_dailyFriend_nextAt', 0" in text and "sched_dailyFriendAfterLunch_nextAt', 0" in text and "sched_dailyBar_nextAt', 0" in text and "facility_cycle_state', null" in text)
