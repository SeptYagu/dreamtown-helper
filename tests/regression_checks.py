...
require("v3.56 metadata and shared panel version", "// @version      3.56" in text and "const SCRIPT_VERSION = '3.56';" in text and "v${SCRIPT_VERSION}" in text)
...
require("scoped daily projects bypass only their own switch", "scope?.startsWith('project_')" in text and "if (scopedProject !== id) return 0;" in text and "actionState.actionProjectId === id" in text)
require("repeatable project actions use explicit per-run success progress", "recordScopedProjectSuccess" in text and "actionState.actionCompleted" in text and "actionProjectId" in text and "actionTarget" in text)
...
require("panel groups daily projects as one continuously numbered item", "dailyProjectModules = ['dailyNpc', 'dailyFriend', 'dailyBar', 'extraWish']" in text and "${displayStep}. 每日项目" in text and "见上方每日项目配置" in text)
require("grouped daily-project row highlights all four internal steps", "row.dataset.modules = dailyProjectModules.join(',');" in text and "modules.includes(currentModule)" in text)
